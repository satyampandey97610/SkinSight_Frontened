...
# Alternative: POST endpoint for simple JSON data (without file upload)
@app.post("/api/submit-json")
async def handle_json_data(
    name: str = Form(...),
    email: str = Form(...),
    message: str = Form(...)
):
    """
    Handle simple form data without file upload
    """
    return {
        "status": "success",
